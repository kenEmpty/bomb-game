...
const Shop = {
  render() {
    ...
    for (const def of SKIN_DEFS) {
      ...
      const canAfford = !def.achievementOnly && pts >= def.price;
      ...
      } else if (def.achievementOnly) {
        actionHtml = `<button class="ghost-btn shop-sm-btn" disabled>🏅<br><small>実績で解放</small></button>`;
      } else if (canAfford) {
      ...
      if (isOwned) {
        ...
        metaHtml = `<div class="skin-badge ach-only-badge">${def.unlockHint || '実績で解放'}</div>`;
      } else if (def.price === 0) {
